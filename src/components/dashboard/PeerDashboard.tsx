...
const PeerDashboard = () => {
  ...
  const acceptInvite = async (requestId: string, roomId: string) => {
    try {
          ...
          >
            {pending.length === 0 ? (
              ...
              pending.map(req => (
                <ListItem
                  ...
                  secondaryAction={
                    <Button
                      ...
                      size='small'
                      onClick={() =>
                        acceptInvite(req.id, `${req.fromId}_${req.toId}`)
                      }
                      sx={{
                        textTransform: 'none',
                        fontSize: {
                ...
                >
                  <ListItemText
                    primary={req.fromUser?.email || 'Unknown'}
          ...
          <List dense>
            {peers.length === 0 ? (
              ...
              peers.map(peer => (
                <ListItem
                  key={peer.id}
                  sx={{ borderBottom: '1px dashed #16f09822', py: 1.5, px: 1 }}
                >
                  <ListItemText
                    primary={peer.name || peer.email}
                    primaryTypographyProps={{
                      color: '#16f098',
                      fontWeight: 500,
                      fontSize: {
